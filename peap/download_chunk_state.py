...
def resolve_chunk_state_path(args: object, *, default_dir: str) -> str:
    ...
    stem = (
        ...
        f"{_safe_state_token(getattr(args, 'record_family', None))}_"
        f"{_safe_state_token(getattr(args, 'business_id', None))}_"
...
def load_chunk_state(path: str) -> ChunkStateContext:
    ...
    if os.path.isfile(path):
        ...
        if not isinstance(raw, dict):
            raise ValueError(f"invalid chunk state format: {path}")
        payload = raw
