...
def _check_output_config(config_obj: object) -> List[CheckResult]:
    ...
    output_contract_keys = set(config_obj.OUTPUT_FILE_NAMES)
    deal_contract_keys = set(config_obj.DEAL_FILE_NAMES)

    if output_contract_keys.issubset(set(config_obj.OUTPUT_FILES.keys())):
        results.append(CheckResult("output-files", True, "output file config complete"))
    else:
        missing = sorted(output_contract_keys - set(config_obj.OUTPUT_FILES.keys()))
        results.append(CheckResult("output-files", False, f"missing output keys: {missing}"))

    if deal_contract_keys.issubset(set(config_obj.DEAL_FILES.keys())):
        results.append(CheckResult("deal-files", True, "deal file config complete"))
    else:
        missing = sorted(deal_contract_keys - set(config_obj.DEAL_FILES.keys()))
