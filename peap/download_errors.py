...
def duplicate_download_target_error(*, source_id: str, task_id: str, raw_reason: str) -> DownloadError:
    normalized_source = _normalized_source_id(source_id)
    normalized_task_id = _normalized_task_id(task_id)
    normalized_reason = _normalized_reason(raw_reason)
    return DownloadError(
        error_code=f"{normalized_source}_duplicate_download_target",
        error_message=f"{normalized_source}: duplicate-download-target: {normalized_reason}",
        stage="save_pages",
        failure_kind="validation",
        source_id=normalized_source,
        task_id=normalized_task_id,
...
def unaccounted_candidates_error(*, source_id: str, task_id: str, raw_reason: str) -> DownloadError:
    normalized_source = _normalized_source_id(source_id)
    normalized_task_id = _normalized_task_id(task_id)
    normalized_reason = _normalized_reason(raw_reason)
    return DownloadError(
        error_code=f"{normalized_source}_unaccounted_download_candidates",
        error_message=f"{normalized_source}: unaccounted-download-candidates: {normalized_reason}",
        stage="save_pages",
        failure_kind="validation",
        source_id=normalized_source,
        task_id=normalized_task_id,
...
def summary_failure_count_error(*, source_id: str, task_id: str, raw_reason: str) -> DownloadError:
    normalized_source = _normalized_source_id(source_id)
    normalized_task_id = _normalized_task_id(task_id)
    normalized_reason = _normalized_reason(raw_reason)
    return DownloadError(
        error_code=f"{normalized_source}_summary_failure_count",
        error_message=f"{normalized_source}: summary-failure-count: {normalized_reason}",
        stage="save_pages",
        failure_kind="validation",
        source_id=normalized_source,
        task_id=normalized_task_id,
...
def archive_audit_failed_error(*, source_id: str, task_id: str, raw_reason: str) -> DownloadError:
    normalized_source = _normalized_source_id(source_id)
    normalized_task_id = _normalized_task_id(task_id)
    normalized_reason = _normalized_reason(raw_reason)
    return DownloadError(
        error_code=f"{normalized_source}_archive_audit_failed",
        error_message=f"{normalized_source}: archive-audit-failed: {normalized_reason}",
        stage="save_pages",
        failure_kind="validation",
        source_id=normalized_source,
        task_id=normalized_task_id,
