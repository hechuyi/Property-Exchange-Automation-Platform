...
class TpreProjectDownloader:
        ...
        for query in self.list_queries:
            ...
            while page_index <= total_pages:
                ...
                try:
                    ...
                    summary.errors.append(f"list-{query.label}-page-{page_index}-request-failed: {exc}")
                    break

                summary.pages_requested += 1
                if int(payload.get("code", -1)) != 0:
                    summary.errors.append(
                        f"list-{query.label}-page-{page_index}-failed: {payload.get('message')}"
                    )
                    break
                ...
                if not isinstance(rows, list):
                    summary.errors.append(f"list-{query.label}-page-{page_index}-invalid-data")
                    break
        ...
        *,
        ...
        for row in rows:
            ...
            if not project_code:
                ...
                summary.errors.append(f"list-{query.label}-missing-project-code")
                continue
            ...
            list_disclosure_start = _parse_date(row.get("startTime"))
            ...
            if not page_url:
                summary.errors.append(f"list-{query.label}-missing-page-url: project_code={project_code}")
                continue
    ...
    def _build_prefetched_candidates(
        ...
        *,
        ...
        for index, raw in enumerate(prefetched_candidates, start=1):
            ...
            if not project_code:
                summary.skipped_by_missing_xmid += 1
                summary.errors.append(f"prefetched-entry-{index}-missing-project-code")
                continue
            if project_code in seen_codes:
                summary.skipped_by_duplicate += 1
                continue
            ...
            list_disclosure_start = _parse_date(raw.get("list_disclosure_start") or row.get("startTime"))
            if (start or end) and list_disclosure_start and not _in_range(list_disclosure_start, start, end):
                summary.skipped_by_list_date += 1
                continue
            ...
            if not page_url:
                summary.errors.append(
                    f"prefetched-entry-{index}-missing-page-url: project_code={project_code}"
                )
                continue
    ...
    async def _process_candidate(
        ...
        *,
        ...
        for attempt in range(1, self._detail_retries + 2):
            ...
            try:
                ...
                if attempt <= self._detail_retries:
                    ...
                    summary.errors.append(f"project_code={candidate.project_code} page-timeout: {exc}")
            except Exception as exc:  # noqa: BLE001
                last_exc = exc
                if attempt <= self._detail_retries:
                    await asyncio.sleep(1.2 * attempt)
                else:
                    summary.errors.append(f"project_code={candidate.project_code} page-fetch-failed: {exc}")
            finally:
                await page.close()
        ...
        list_start = _parse_date(candidate.row.get("list_disclosure_start") or candidate.row.get("startTime"))
        if (start or end):
        ...
        try:
            ...
            summary.detail_failed += 1
            summary.errors.append(f"project_code={candidate.project_code} save-failed: {exc}")
            return
