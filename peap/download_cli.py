...
def build_parser(config_obj: object | None = None) -> argparse.ArgumentParser:
    ...
    family_choices = [descriptor.family_id for descriptor in list_family_descriptors()]
    default_family = str(defaults.get("record_family", "listing") or "listing")
    task_exchange_choices = exchange_choices(resolved_config, record_family="all")
    task_business_choices = business_choices(record_family="all")
    parser = argparse.ArgumentParser(description="Automated page downloader")
    parser.add_argument(
        "--record-family",
        choices=family_choices,
        default=default_family,
        help="Target record family",
    )
    parser.add_argument(
        "--exchange",
    ...
    parser.add_argument(
        "--business-id",
        choices=task_business_choices + ["all"],
        default=str(defaults.get("business_id", "all") or "all"),
        help="Target business id (or all registered businesses in the selected family)",
    )
    parser.add_argument(
        "--list-tasks",
        action="store_true",
