...
from .download_artifact_audit import TaskArtifactAudit
...
def _summary_explicit_failure_count_message(summary: object) -> str | None:
    for attr in (
        "detail_failed",
        "list_failed",
        "save_failed",
        ...
        try:
            ...
            continue
        if value > 0:
            return f"summary explicit failure count: {attr}={value}"
    return None
...
def execute_split_task(
    *,
    ...
    artifact_audit: TaskArtifactAudit | None = None,
) -> DownloadMaterializeResult:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        chunk_last_error = (
            str(chunk_state.get("last_error") or "").strip()
            if chunk_state is not None
            else ""
        )
        stale_chunk = bool(artifact_audit and artifact_audit.intersects(chunk.start, chunk.end))
        if chunk_state is not None and chunk_status == "done" and not stale_chunk and not chunk_last_error:
            print(
            ...
            continue
        if chunk_state is not None and chunk_status == "done" and chunk_last_error:
            print(
                f"[{spec.task_id}] Recheck chunk {idx}/{len(chunks)} "
                f"{chunk.start_str}..{chunk.end_str} status=done last_error_present"
            )
            logger.warning(
                "[%s] Recheck chunk %s/%s %s..%s status=done last_error_present",
                spec.task_id,
                idx,
                len(chunks),
                chunk.start_str,
        ...
        if chunk_state is not None and chunk_status == "done" and stale_chunk:
            print(
                f"[{spec.task_id}] Recheck chunk {idx}/{len(chunks)} "
                f"{chunk.start_str}..{chunk.end_str} status=done stale_download_record"
            )
            logger.warning(
                "[%s] Recheck chunk %s/%s %s..%s status=done stale_download_record",
                spec.task_id,
                idx,
                len(chunks),
                chunk.start_str,
        ...
        if skip_zero_chunks and chunk.estimated_candidates <= 0 and not stale_chunk:
            print(
                f"[{spec.task_id}] Skip chunk {idx}/{len(chunks)} "
            ...
            if task_chunk_state is not None and chunk_state_ctx is not None:
                update_chunk_state(task_chunk_state, chunk, status="skipped_zero_estimate")
        ...
        if task_chunk_state is not None and chunk_state_ctx is not None:
            ...
            save_chunk_state(chunk_state_ctx)
        try:
            force_chunk_rerun = task_chunk_state is not None or stale_chunk
            if force_chunk_rerun and args.resume:
                logger.info(
                    "[%s] Chunk %s/%s runs with resume disabled due to artifact audit or chunk-state recovery priority",
                    spec.task_id,
                    idx,
                    len(chunks),
                )
        ...
        typed_summary_errors = summary_typed_errors(summary)
        explicit_failure_message = _summary_explicit_failure_count_message(summary)
        summary_failed = bool(typed_summary_errors) or explicit_failure_message is not None
        ...
        if task_chunk_state is not None and chunk_state_ctx is not None:
            if summary_failed:
                error_message = (
                    typed_summary_errors[0].error_message
                    if typed_summary_errors
                    else explicit_failure_message
                )
                update_chunk_state(
                    task_chunk_state,
                    chunk,
