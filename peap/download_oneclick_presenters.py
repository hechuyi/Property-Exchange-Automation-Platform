...
def _summary_payload_dict(summary_payload: Mapping[str, Any] | None) -> dict[str, Any]:
    if summary_payload is None:
        return {}
    if not isinstance(summary_payload, Mapping):
        raise TypeError("summary_payload must be a mapping or None")
    return dict(summary_payload)



def stage_error_message(summary_payload: dict[str, Any] | None) -> str:
    payload = _summary_payload_dict(summary_payload)
...
def emit_stage(
    ...
    *,
    ...
    if callback is None:
        return
    summary_payload_dict = _summary_payload_dict(summary_payload)
    payload = {
        "phase_code": str(phase_code),
        "status": str(status),
        "label": str(label),
        "summary_payload": summary_payload_dict,
    }
    error_message = stage_error_message(summary_payload_dict)
    if error_message:
