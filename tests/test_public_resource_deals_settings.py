...
from peap_parsers.public_resource import parse_mhtml_file as canonical_parse_mhtml_file
...
class PublicResourceDealSettingsTest(unittest.TestCase):
    ...
    def test_workbook_and_parser_use_one_canonical_mhtml_implementation(self) -> None:
        self.assertIs(parse_mhtml_file, canonical_parse_mhtml_file)
    ...
    def test_mhtml_html_part_rejects_invalid_bytes_for_declared_charset(self) -> None:
        input_dir = Path(self.temp_dir.name) / "bad-mhtml"
        input_dir.mkdir(parents=True, exist_ok=True)
        file_path = input_dir / "bad.mhtml"
        file_path.write_bytes(
            b"\r\n".join(
                [
                    ...
                    b"Content-Location: https://example.invalid/bad.html",
                    b"",
                    b"<html><body>invalid bytes: \xff\xff</body></html>",
                    b"--BOUNDARY--",
                    b"",
                ]
            )
        ...
        with self.assertRaises(UnicodeDecodeError):
            parse_mhtml_file(str(file_path))

    def test_build_workbook_records_decode_failure_instead_of_success_row(self) -> None:
        input_dir = Path(self.temp_dir.name) / "bad-workbook-mhtml"
        input_dir.mkdir(parents=True, exist_ok=True)
        file_path = input_dir / "bad.mhtml"
        file_path.write_bytes(
            b"\r\n".join(
        ...
        output_file = Path(self.temp_dir.name) / "excel" / "decode-failure.xlsx"
        ...
        self.assertEqual(summary.failed[0].file_path, str(file_path))
        self.assertIn("codec can't decode byte 0xff", summary.failed[0].error)
