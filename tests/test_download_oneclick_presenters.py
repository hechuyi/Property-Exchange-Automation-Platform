...
class DownloadOneClickPresentersTest(unittest.TestCase):
    ...
    def test_summary_payload_rejects_non_mapping_top_level_values(self) -> None:
        emitted: list[dict[str, object]] = []

        self.assertEqual(stage_error_message(None), "")
        emit_stage(
            emitted.append,
            phase_code="prepare_tasks",
            status="running",
        ...
        self.assertEqual(emitted[0]["summary_payload"], {})

        for bad_payload in ([], "", [("error_message", "pair sequence boom")]):
            with self.subTest(bad_payload=bad_payload):
                with self.assertRaises(TypeError):
                    stage_error_message(bad_payload)  # type: ignore[arg-type]
                with self.assertRaises(TypeError):
                    emit_stage(
                        emitted.append,
                        phase_code="prepare_tasks",
                        status="failed",
                        label="扫描失败",
                        summary_payload=bad_payload,  # type: ignore[arg-type]
                    )
