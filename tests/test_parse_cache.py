...
class ParseCacheContractTest(unittest.TestCase):
    ...
    def test_parse_cache_rehydrates_standard_record(self) -> None:
        ...
        self.assertEqual(cached.standard_record.business_type, TYPE_EQUITY_TRANSFER)
        ...
        self.assertEqual(cached.business_type, TYPE_EQUITY_TRANSFER)
    ...
    def test_parse_cache_prefers_structured_parsed_project_payload(self) -> None:
        ...
        structured_payload["standard_record"]["business_type"] = TYPE_EQUITY_TRANSFER
...
class ParseCacheRegressionTest(unittest.TestCase):
    ...
    def test_parse_cache_propagates_parsed_project_contract_errors(self) -> None:
        """Valid JSON cache rows must not disguise deserialization contract errors."""
        html_file = os.path.join(self.temp_dir.name, "contract_error.html")
        with open(html_file, "w", encoding="utf-8") as handle:
            handle.write("<html></html>")

        store = ParseCacheStore(
            db_path=os.path.join(self.temp_dir.name, "parse_cache_contract.sqlite3"),
            run_signature="test-signature-contract",
            commit_interval=1,
        )
        self.addCleanup(store.close)
        ...
        with patch(
            "peap.parse_cache.ParsedProject.from_cache_payload",
            side_effect=TypeError("standard_record.project_code must be text"),
        ):
            with self.assertRaisesRegex(TypeError, "project_code"):
                store.get(html_file)
