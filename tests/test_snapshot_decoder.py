...
class SnapshotDecoderTest(unittest.TestCase):
    ...
    def test_decode_html_snapshot_rejects_bytes_that_are_not_utf8_or_gb18030(self) -> None:
        from peap_parsers.snapshot_decoder import decode_snapshot

        snapshot = SnapshotEnvelope(
            snapshot_id="snap-bad-html",
            captured_at="2026-03-31T10:00:00Z",
            source_url="https://example.invalid/listing/bad",
            referrer_url="https://example.invalid/listings",
            content_type="text/html",
            http_status=200,
            storage_path="/tmp/snap-bad-html.html",
            digest="sha256:bad-html",
            fetch_metadata={"method": "GET"},
        )
        raw_content = b"<html><body>invalid bytes: \xff\xff</body></html>"

        with self.assertRaises(UnicodeDecodeError):
            decode_snapshot(snapshot, raw_content=raw_content)

    def test_mhtml_html_part_rejects_wrong_charset_with_invalid_bytes(self) -> None:
        from peap_parsers.snapshot_decoder import decode_snapshot, load_html_parts_from_mhtml
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            file_path = Path(temp_dir) / "bad-fixture.mhtml"
            file_path.write_bytes(mhtml)
            snapshot = SnapshotEnvelope(
                snapshot_id="snap-bad-mhtml",
                captured_at="2026-03-31T10:00:00Z",
                source_url="https://example.invalid/bad.html",
                referrer_url="https://example.invalid/",
                content_type="multipart/related",
                http_status=200,
                storage_path=str(file_path),
                digest="sha256:bad-mhtml",
                fetch_metadata={"method": "GET"},
            )

            with self.assertRaises(UnicodeDecodeError):
                load_html_parts_from_mhtml(file_path)
            with self.assertRaises(UnicodeDecodeError):
