...
from peap_core.runtime import load_runtime_config, resolve_path, resolve_runtime_data_root
...
MAPPING_SOURCES = {"excel_only", "excel_then_metadata"}


class SubmissionConfigurationError(ValueError):
    """Raised when submission configuration is invalid."""
...
class SubmissionConfig:
    ...
    raw_source_roots: Tuple[Tuple[str, str], ...] = ()

    def source_roots(self) -> Tuple[Tuple[str, str], ...]:
        if self.raw_source_roots:
            return tuple(
                (str(source_kind or "other").strip().lower(), os.path.abspath(path))
                for source_kind, path in self.raw_source_roots
                if str(path or "").strip()
            )
        return (("", os.path.abspath(self.raw_base_dir)),)

    def validate(self) -> bool:
        available_source_roots = [
            path for _source_kind, path in self.source_roots() if os.path.isdir(path)
        ]
        if not available_source_roots:
            logger.error(
                "源页面目录不存在："
                + ", ".join(path for _source_kind, path in self.source_roots())
            )
            return False
        if not os.path.isdir(self.output_excel_dir) and self.mapping_source == "excel_only":
            ...
            return False
        if self.mapping_source not in MAPPING_SOURCES:
...
def _parse_bool(value, default: bool) -> bool:
    ...
    if isinstance(value, str):
        ...
        if token in {"0", "false", "no", "off"}:
            return False
    raise SubmissionConfigurationError(f"布尔配置无法解析：{value!r}")
...
def load_config_from_runtime() -> SubmissionConfig:
    ...
    data_root = resolve_runtime_data_root(
        str(paths.get("data_root") or ""),
        project_root=project_root,
    )
    manual_root = resolve_path(
        str(paths.get("html_folder") or "raw/manual"),
        base_dir=data_root,
    )
    auto_root = resolve_path(
        str(paths.get("auto_html_folder") or "raw/auto"),
        base_dir=data_root,
    )
    output_excel_dir = resolve_path(
        str(paths.get("output_excel_dir") or "outputs/excel"),
        base_dir=data_root,
    )
    submission_dir = resolve_path(
        str(paths.get("submission_dir") or "outputs/submission"),
        base_dir=data_root,
    )
    log_dir = resolve_path(
        str(paths.get("log_dir") or "logs"),
        base_dir=data_root,
    )
    submission_defaults = dict(payload.get("submission_defaults") or {})

    raw_mapping_source = submission_defaults.get("mapping_source", "excel_only")
    if raw_mapping_source is None:
        raw_mapping_source = "excel_only"
    mapping_source = str(raw_mapping_source).strip().lower()
    if mapping_source not in MAPPING_SOURCES:
        raise SubmissionConfigurationError(
            f"submission_defaults.mapping_source 无效：{raw_mapping_source!r}"
        )
    ...
    try:
        ...
        raise SubmissionConfigurationError(
            f"submission_defaults.filename_max_bytes 无法解析：{filename_max_bytes!r}"
        ) from exc
    if filename_max_bytes < 80:
        raise SubmissionConfigurationError(
            f"submission_defaults.filename_max_bytes 过小：{filename_max_bytes}"
        )
    ...
    source_roots = (
        (("auto", auto_root), ("manual", manual_root))
        if prefer_auto
        else (("manual", manual_root), ("auto", auto_root))
    )
    try:
        raw_base_dir = os.path.commonpath([manual_root, auto_root])
    except ValueError:
        raw_base_dir = data_root

    return SubmissionConfig(
        data_root=data_root,
        raw_base_dir=raw_base_dir,
        output_dir=os.path.dirname(submission_dir),
        ...
        raw_source_roots=source_roots,
    )


def select_excel_files(output_excel_dir: str, *, required: bool = False) -> List[str]:
    if not os.path.isdir(output_excel_dir):
        if required:
            raise SubmissionConfigurationError(f"Excel 目录不存在：{output_excel_dir}")
...
def load_mapping_from_source_roots(
    source_roots: Iterable[Tuple[str, str]],
) -> Dict[str, ProjectMapping]:
    mapping: Dict[str, ProjectMapping] = {}
    for _source_kind, source_root in source_roots:
        if not os.path.isdir(source_root):
            continue
        for project_code, candidate in load_mapping_from_metadata(source_root).items():
            mapping.setdefault(project_code, candidate)
    return mapping


def build_mapping(config: SubmissionConfig) -> Dict[str, ProjectMapping]:
    excel_paths = select_excel_files(
        config.output_excel_dir,
        required=config.mapping_source == "excel_only",
    )
    ...
    metadata_mapping = load_mapping_from_source_roots(config.source_roots())
...
def scan_source_pages(
    raw_base_dir: str,
    *,
    source_roots: Iterable[Tuple[str, str]] | None = None,
) -> Tuple[List[SourcePage], List[str]]:
    pages: List[SourcePage] = []
    unresolved: List[str] = []

    configured_roots = tuple(source_roots or (("", raw_base_dir),))
    all_paths: List[Tuple[str, str, str]] = []
    seen_paths: set[str] = set()
    for explicit_kind, source_root in configured_roots:
        if not os.path.isdir(source_root):
            continue
        for root, _, files in os.walk(source_root):
            for file_name in files:
                ...
                normalized_path = os.path.normcase(source_path)
                if normalized_path in seen_paths:
                    continue
                seen_paths.add(normalized_path)
                all_paths.append((explicit_kind, source_root, source_path))

    all_paths.sort(
        key=lambda item: (
            str(item[0] or ""),
            os.path.relpath(item[2], item[1]).replace("\\", "/").lower(),
        )
    )

    for explicit_kind, source_root, source_path in all_paths:
        exact_code, metadata_name = _resolve_exact_code(source_path)
        if not exact_code:
            unresolved.append(source_path)
            continue

        root_relative_path = os.path.relpath(source_path, source_root).replace("\\", "/")
        if explicit_kind:
            source_kind = str(explicit_kind).strip().lower()
            rel_path = f"{source_kind}/{root_relative_path}"
        else:
...
def run_submission_prepare(config: SubmissionConfig) -> Tuple[int, int, int, List[str], List[ManifestRecord]]:
    ...
    source_pages, unresolved_sources = scan_source_pages(
        config.raw_base_dir,
