...
class BeijingStandardParser(WebPageParser):
    ...
    _PHYSICAL_TITLE_SELLER_RE = re.compile(
        r"^\s*(?P<seller>[A-Za-z0-9\u4e00-\u9fff（）()·&\-\s]{2,120}?"
        r"(?:有限责任公司|股份有限公司|集团有限公司|有限公司))"
        r"(?=(?:拟(?:处置|转让|出售|报废)|处置|转让|出售|报废|固定资产|废旧?物资|废旧?设备|"
        r"废电子设备|办公电子设备|机器设备|设备|车辆|房产|资产))"
    )
        ...
        return self.build_standard_payload_from_data(
            {
                ...
                "share_ratio": "持股比例",
                "remark": "备注",
                "state_asset_supervisor": "国资监管机构",
                "state_funded_department": "国家出资企业或主管部门名称",
                "economic_type": "经济类型",
                "seller_credit_code": ("转让方统一社会信用代码", "统一社会信用代码或组织机构代码", "统一社会信用代码"),
            }
        )
    ...
    @staticmethod
    def _infer_project_type_from_code(project_code: Any) -> str:
        code = str(project_code or "").strip().upper()
        if not code:
            return ""
        if code.endswith("-0"):
            return "预披露"
        if code.startswith(("GR", "GF", "TA", "TR")):
            return "实物资产"
    ...
    @classmethod
    def _infer_physical_seller_from_project_name(
        cls,
        project_code: Any,
        project_name: Any,
    ) -> str:
        if not cls._is_physical_code(project_code):
            return ""
        match = cls._PHYSICAL_TITLE_SELLER_RE.match(str(project_name or "").strip())
        if not match:
            return ""
        return re.sub(r"\s+", "", match.group("seller")).strip()

    @staticmethod
    def _normalize_table_label(value: Any) -> str:
        return re.sub(r"\s+", "", str(value or "").strip())

    def _extract_seller_condition_field(self, label: str) -> str:
        target = self._normalize_table_label(label)
        if not target:
            return ""
        seller_condition = self.soup.find("table", id="sellercondition")
        scopes = [seller_condition] if seller_condition is not None else [self.soup]
        for scope in scopes:
            if scope is None:
                continue
            for row in scope.find_all("tr"):
                ...
                for index, cell_text in enumerate(cells):
                    if self._normalize_table_label(cell_text) != target:
                        continue
    ...
    def _is_placeholder_org_value(value: Any) -> bool:
        ...
        if not text:
            return True
        normalized_dash = text.replace("-", "").replace("－", "").replace("—", "")
        if not normalized_dash:
            return True
        return text in {"无", "无主管单位", "不涉及", "暂无", "未知"}
    ...
    def _pick_physical_group_name(self, seller: Dict[str, Any], seller_name: str) -> str:
        ...
        - 审批单位为空/占位，或审批单位组合中包含 hqname 时，优先取 hqname；
        - 其他场景优先审批单位。
        """
        ...
        hq_name = "" if self._is_placeholder_org_value(seller.get("hqname", "")) else str(seller.get("hqname", "") or "").strip()
        seller_text = str(seller_name or "").strip()

        if not self._is_placeholder_org_value(authorize):
            ...
            if hq_name and hq_name in auth:
                return hq_name
    ...
    def _extract_bj_physical_category_from_sidecar(self) -> str:
        ...
        if sidecar.is_symlink():
            raise ValueError(f"beijing physical category sidecar must not be a symlink: {sidecar}")
        if not sidecar.exists():
            return ""
        payload = json.loads(sidecar.read_text(encoding="utf-8"))
        if not isinstance(payload, dict):
            raise ValueError("beijing physical category sidecar must be an object")

        row = payload.get("row")
        if row is not None and not isinstance(row, dict):
            raise ValueError("beijing physical category sidecar.row must be an object")
    ...
    def _parse_from_json(self, json_data: Dict) -> None:
        ...
        official_project_schema = any(
            isinstance(json_data.get(key), dict)
            for key in (
                "utrgcemsproject",
                "utrmcemsproject",
                "utrzcemsproject",
        ...
        inferred_project_type = self._infer_project_type_from_code(project_code)
        if official_project_schema and inferred_project_type and not self.data.get("项目类型"):
            self.data["项目类型"] = inferred_project_type
        ...
        seller_supervisor = self._extract_seller_condition_field("国资监管机构")
        if seller_supervisor:
            self.data["国资监管机构"] = seller_supervisor
        state_funded_department = self._extract_seller_condition_field("国家出资企业或主管部门名称")
        if state_funded_department:
            self.data["国家出资企业或主管部门名称"] = state_funded_department
        economic_type = self._extract_seller_condition_field("经济类型")
        if economic_type:
            self.data["经济类型"] = economic_type
        seller_credit_code = self._first_non_empty(
            self._extract_seller_condition_field("统一社会信用代码或组织机构代码"),
            self._extract_seller_condition_field("统一社会信用代码"),
            seller.get("sellercode", ""),
        )
        if seller_credit_code:
            self.data["统一社会信用代码或组织机构代码"] = seller_credit_code
    ...
    def _parse_from_html(self) -> None:
        ...
        if not self.data.get('转让方'):
            inferred_seller = self._infer_physical_seller_from_project_name(
                self.data.get('项目编号'),
                self.data.get('项目名称'),
            )
            if inferred_seller:
                self.data['转让方'] = inferred_seller
