...
class GuangzhouParser(WebPageParser):
    ...
    EXCHANGE_NAME = "\u5e7f\u4ea4\u6240"
    PROJECT_CODE_PATTERN = re.compile(r"(?:G[36R]|Q[36R]|T[36R])\d{4}(?:GD|GZ)\d+(?:-\d+)?")
            ...
            for row_idx, row in enumerate(rows):
                ...
                for col_idx, cell in enumerate(cells):
                    ...
                    if not label:
                        continue
                    next_text = (
                        self._cell_text(cells[col_idx + 1]).strip()
                    ...
                    if (
                        (
                            self.KEY_INDUSTRY not in self.data
                            or self._is_empty_value(self.data.get(self.KEY_INDUSTRY))
                        )
                        and label
                        in {
                            "\u6240\u5c5e\u884c\u4e1a",
                    ...
                    if (
                        self.KEY_PROFIT not in self.data
                        or self._is_empty_value(self.data.get(self.KEY_PROFIT))
                        ...
                        for step in (1, 2, 3):
                            ...
                            for target_idx in target_indices:
                                ...
                                if number is not None:
                                    self.data[self.KEY_PROFIT] = number
                                    break
                if self.KEY_PROFIT in self.data and not self._is_empty_value(
                    self.data.get(self.KEY_PROFIT)
                ):
                    break
    ...
    def _supplement_from_remote_tabs(self, script_vars: Dict[str, str]) -> None:
        ...
        needs_contact = self.KEY_CONTACT not in self.data or self._is_empty_value(
            self.data.get(self.KEY_CONTACT)
        )
        needs_industry = self.KEY_INDUSTRY not in self.data or self._is_empty_value(
            self.data.get(self.KEY_INDUSTRY)
        )
        needs_profit = self.KEY_PROFIT not in self.data or self._is_empty_value(
            self.data.get(self.KEY_PROFIT)
        )
        needs_group = (
            self.KEY_GROUP not in self.data
            or self._is_empty_value(self.data.get(self.KEY_GROUP))
            or group_same_as_seller
        )
        ...
        if needs_industry or needs_profit:
            for tab in ("compInfo", "home"):
                ...
                has_industry = self.KEY_INDUSTRY in self.data and not self._is_empty_value(
                    self.data.get(self.KEY_INDUSTRY)
                )
                has_profit = self.KEY_PROFIT in self.data and not self._is_empty_value(
                    self.data.get(self.KEY_PROFIT)
                )
    ...
    def _extract_from_top_summary(self) -> None:
        ...
        # Current rendered pages use a labelled price box instead of the
        # legacy ``span.price`` selector.  Match the label and its paired
        # value so the adjacent guarantee amount is not selected.
        if self.KEY_LISTING_PRICE not in self.data:
            for box in self.soup.select(".project-price-box"):
                labels = box.select(".project-price-name")
                values = box.select(".project-price-num")
                for index, label in enumerate(labels):
                    if "\u8f6c\u8ba9\u5e95\u4ef7" not in self._norm(self._cell_text(label)):
                        continue
                    if index >= len(values):
                        continue
                    price = self.clean_price(self._cell_text(values[index]))
                    if price is not None:
                        self.data[self.KEY_LISTING_PRICE] = price
                        break
                if self.KEY_LISTING_PRICE in self.data:
                    break
    ...
    def _extract_project_name_from_title(self) -> str:
        title = self.soup.find("title")
        title_text = self._cell_text(title).strip() if title else ""
        if not title_text:
            return ""
        generic_tokens = (
            "广东联合产权交易中心",
    ...
    def _is_seller_label(norm_label: str) -> bool:
        return bool(
            re.fullmatch(
                r"\u8f6c\u8ba9\u65b9(?:[\u4e00-\u9fa5\d]+)?\u540d\u79f0", str(norm_label or "")
            )
        )
            ...
            for row in table.find_all("tr"):
                ...
                for idx, cell in enumerate(cells):
                    ...
                    if not label:
                        continue
                    next_text = (
                        self._cell_text(cells[idx + 1]).strip() if idx + 1 < len(cells) else ""
                    )
            ...
            for row_idx, row in enumerate(rows):
                ...
                for col_idx, cell in enumerate(cells):
                    ...
                    if not label:
                        continue
                    next_text = (
                        self._cell_text(cells[col_idx + 1]) if col_idx + 1 < len(cells) else ""
                    )

                    if (
                        label == "\u6807\u7684\u4f01\u4e1a\u540d\u79f0"
                        and next_text
                        and self.KEY_PROJECT_NAME not in self.data
                    ):
                    ...
                    if label == "\u6240\u5728\u5730\u533a" and next_text:
                        self.data[self.KEY_REGION] = self.clean_region(
                            re.sub(r"\s*-\s*", "", next_text)
                        )
                        continue
                    ...
                    if (
                        ...
                        self.data[self.KEY_LISTING_START] = self.clean_date(next_text)
                        continue
                    ...
                    if (
                        ...
                        self.data[self.KEY_LISTING_END] = self.clean_date(next_text)
                        continue
                    ...
                    if (
                        ...
                        price = self.clean_price(next_text)
                        if price is not None and self.KEY_LISTING_PRICE not in self.data:
                            self.data[self.KEY_LISTING_PRICE] = price
                        continue

                    if label in {"\u878d\u8d44\u91d1\u989d", "\u62df\u52df\u96c6\u8d44\u91d1\u603b\u989d"} and next_text:
                        price = self.clean_price(next_text)
                        if price is not None:
                            self.data["\u878d\u8d44\u91d1\u989d"] = price
                            self.data.setdefault(self.KEY_LISTING_PRICE, price)
                        continue
                    ...
                    if (
                        ...
                        and next_text
                        and self.KEY_SHARE_RATIO not in self.data
                    ):
                    ...
                    if (
                        ...
                        self.data[self.KEY_CONTACT] = (
                            self.extract_contact_person(next_text) or next_text
                        )
                        continue
                    ...
                    if (
                        label == "\u8f6c\u8ba9\u65b9\u540d\u79f0"
                        and next_text
                        and self.KEY_SELLER not in self.data
                    ):
                        self.data[self.KEY_SELLER] = next_text
                        continue

                    if (
                        label in {"\u878d\u8d44\u65b9\u540d\u79f0", "\u589e\u8d44\u4f01\u4e1a\u540d\u79f0"}
                        and next_text
                        and self.KEY_SELLER not in self.data
                    ):
                        self.data[self.KEY_SELLER] = next_text
                    ...
                    if (
                        ...
                        and next_text
                        and self.KEY_AGENCY not in self.data
                    ):
                        self.data[self.KEY_AGENCY] = next_text
                        continue

                    if label == "\u603b\u8d44\u4ea7" and next_text:
                        number = self._to_float(next_text)
                        if number is not None:
                            self.data[self.KEY_ASSET_TOTAL] = number
                        continue
        ...
        return self.build_standard_payload_from_data(
            {
                ...
                "business_type": self.KEY_PROJECT_TYPE,
                ...
                "asset_total": self.KEY_ASSET_TOTAL,
    ...
    def parse(self) -> ParserOutput:
        ...
        if self.KEY_PROJECT_NAME not in self.data:
            title_name = self._extract_project_name_from_title()
            if title_name:
                self.data[self.KEY_PROJECT_NAME] = title_name
        ...
        if (
            self.KEY_CONTACT not in self.data
            or not str(self.data.get(self.KEY_CONTACT) or "").strip()
        ):
        ...
        # The detail route is the authoritative distinction on the current
        # Guangdong site.  Older pages may lack the route, so retain code and
        # page-text fallbacks for archived snapshots.
        page_blob = str(self.soup).lower()
        project_code = str(self.data.get(self.KEY_PROJECT_CODE) or "").upper()
        project_name = str(self.data.get(self.KEY_PROJECT_NAME) or "")
        if "capital_increasedetail" in page_blob or "capitalincrease/detail" in page_blob:
            self.data[self.KEY_PROJECT_TYPE] = "增资扩股"
        elif "equitydetail" in page_blob or "equity/detail" in page_blob:
            self.data[self.KEY_PROJECT_TYPE] = "股权转让"
        elif re.match(r"^G62\d+", project_code) or any(
            token in project_name for token in ("增资项目", "增资扩股", "增资")
        ):
            self.data[self.KEY_PROJECT_TYPE] = "增资扩股"
        elif re.match(r"^(?:G32|Q3|T3)\d+", project_code) or any(
            token in project_name for token in ("股权", "股份", "财产份额")
        ):
            self.data[self.KEY_PROJECT_TYPE] = "股权转让"
        ...
        if (
            self.KEY_AGENCY not in self.data
            or not str(self.data.get(self.KEY_AGENCY) or "").strip()
        ):
