...
class ShandongParser(WebPageParser):
    ...
    @classmethod
    def _is_detail_table(cls, table) -> bool:
        table_text = cls._cell_text(table)
        return any(marker in table_text for marker in cls.DETAIL_FIELD_MARKERS)

    @staticmethod
    def _is_error_title(title_text: str) -> bool:
        norm = re.sub(r"\s+", "", str(title_text or ""))
        return norm in {"访问出错", "页面访问出错"} or "访问出错" in norm
    ...
    def parse(self) -> Dict[str, Any]:
        ...
        detail_tables = [table for table in tables if self._is_detail_table(table)]
        ...
        # 项目编号兜底只在详情表范围内搜索，避免错误页推荐区编号污染记录身份。
        if '项目编号' not in self.data:
            for table in detail_tables:
                match = self.PROJECT_CODE_PATTERN.search(self._cell_text(table))
                if match:
                    self.data['项目编号'] = match.group(0)
                    break
        ...
        if title_text and not self._is_error_title(title_text):
