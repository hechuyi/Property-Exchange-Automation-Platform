...
describe("MappingsPage", () => {
  ...
  it("accepts smoke-style controlled input injection for batch draft save", async () => {
    const smokeDomModule = await import("../../smoke_dom.js");
    const smokeDom = (smokeDomModule as any).default ?? smokeDomModule;
    const applyControlledInputValue = smokeDom.applyControlledInputValue as (
    ...
    await act(async () => {
      applyControlledInputValue(targetInput, "测试集团");
    });
    fireEvent.click(screen.getByRole("button", { name: "保存已填写规则" }));
