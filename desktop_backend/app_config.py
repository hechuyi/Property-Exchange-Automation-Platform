...
from peap.download_task_defaults import task_page_size_defaults
...
DEFAULT_DOWNLOADER_TASK_PAGE_SIZE: Dict[str, int] = task_page_size_defaults()
...
def _default_workspace_root() -> str:
    ...
    return normalize_path(os.path.join(_home_dir(), "Documents", "PEAP"))
...
def _legacy_platform_app_home() -> str:
    ...
    return normalize_path(os.path.join(base_dir, "PEAP"))
...
def _legacy_project_browser_cache(project_root: str) -> str:
    return normalize_path(os.path.join(str(project_root or ""), ".cache", "ms-playwright"))
...
def _resolve_env_path(env_name: str, default_value: str) -> str:
    ...
    return normalize_path(default_value)


def _default_archive_root(app_home: str) -> str:
    return normalize_path(os.path.join(str(app_home or ""), "archive"))
...
class AppConfig:
    ...
    def from_env(
        ...
        *,
        ...
        """Build an AppConfig from PEAP_* env vars and platform defaults.

        Set ``ensure_dirs=False`` and ``migrate_legacy=False`` for read-only
        path lookups (e.g. maintenance scripts that just want to know where
        ARCHIVE_ROOT lives). Defaults stay True for the desktop startup path
        so app launch keeps owning directory creation and legacy migration.
        """
        resolved_project_root = normalize_path(
            project_root or os.path.join(os.path.dirname(__file__), "..")
        )
        resolved_app_home = normalize_path(app_home or _default_workspace_root())
        ...
        archive_root = _resolve_env_path("PEAP_ARCHIVE_ROOT", _default_archive_root(resolved_app_home))
    ...
    def migrate_legacy_layout(self) -> "AppConfig":
        ...
        default_documents_root = normalize_path(os.path.join(_home_dir(), "Documents", "PEAP"))
        explicit_documents_root = bool(os.environ.get("PEAP_DOCUMENTS_HOME"))
        owns_default_documents_root = _same_path(self.APP_HOME, default_documents_root)

        migrations = [
            (os.path.join(self.DATA_ROOT, "raw", "manual"), self.HTML_FOLDER),
            (os.path.join(self.DATA_ROOT, "raw", "auto"), self.ARCHIVE_ROOT),
        ]
        if owns_default_documents_root:
        ...
        if explicit_documents_root or owns_default_documents_root:
            legacy_documents_root = _legacy_documents_root()
            migrations.extend(
                [
                    (os.path.join(legacy_documents_root, "archive"), self.ARCHIVE_ROOT),
...
def _merge_tree(source_path: str, target_path: str) -> None:
    ...
    # ``lexists`` is intentional: a dangling symlink is still an invalid
    # migration input and must not be silently treated as absent.
    if not source or not target or source == target or not os.path.lexists(source):
        return
    # Legacy migration is an input boundary. Following a symlink here could
    # move external content into the new workspace or overwrite it indirectly.
    if os.path.islink(source):
        raise ValueError(f"legacy migration refuses symlink source: {source}")
    if os.path.lexists(target) and os.path.islink(target):
        raise ValueError(f"legacy migration refuses symlink target: {target}")
    ...
    for name in os.listdir(source):
        ...
        child_target = os.path.join(target, name)
        if os.path.islink(child_source):
            raise ValueError(f"legacy migration refuses symlink source: {child_source}")
        if os.path.lexists(child_target) and os.path.islink(child_target):
            raise ValueError(f"legacy migration refuses symlink target: {child_target}")
        _merge_tree(child_source, child_target)
    try:
