...
def _normalize_record_families(raw_value: Any) -> tuple[str, ...]:
    ...
    if isinstance(raw_value, str):
        ...
        raise ValueError("record_families must be a string or array")
    ...
    for item in values:
        ...
        family_id = descriptor.family_id if descriptor is not None else text.lower()
...
def _read_record_families(raw_setting: Dict[str, Any], base: RuleSettings | None = None) -> tuple[str, ...]:
    base_families = () if base is None else tuple(base.record_families)
    for key in ("record_families", "families", "record_family", "family"):
        if key in raw_setting:
            return _normalize_record_families(raw_setting.get(key))
    return base_families


def _read_rule_params(
    raw_setting: Dict[str, Any],
    *,
    base_params: Dict[str, Any] | None = None,
    field_name: str,
) -> Dict[str, Any]:
    if "params" not in raw_setting:
        return dict(base_params or {})
    params = raw_setting.get("params")
    if params is None:
        return {}
    ...
    if isinstance(raw_rules, list):
        ...
        for idx, item in enumerate(raw_rules):
            ...
            if isinstance(item, dict):
                ...
                if not rule_id:
                    raise ValueError(f"rules[{idx}].id is required")
                listed[rule_id] = RuleSettings(
                    enabled=bool(item.get("enabled", True)),
                    priority=int(item.get("priority", (idx + 1) * 10)),
                    params=_normalize_rule_params(
                        base_dir,
                        _read_rule_params(item, field_name=f"rules[{idx}].params"),
                    ),
                    record_families=_read_record_families(item),
                )
        return listed
    ...
    for rule_id, raw_setting in raw_rules.items():
        if isinstance(raw_setting, bool):
            ...
            rules[rule_id] = RuleSettings(
                ...
                record_families=tuple(base.record_families),
            )
            continue
        ...
        rules[rule_id] = RuleSettings(
            ...
            params=_normalize_rule_params(
                ...
                _read_rule_params(
                    ...
                    field_name=f"rules.{rule_id}.params",
                ),
            ),
            record_families=_read_record_families(raw_setting, base),
        )
    return rules
...
def load_rules_config(config_path: str) -> Dict[str, RuleSettings]:
    payload = _load_payload(config_path)
    base_dir = os.path.dirname(os.path.abspath(config_path))
    return _parse_rules(payload.get("rules"), base_dir=base_dir)
