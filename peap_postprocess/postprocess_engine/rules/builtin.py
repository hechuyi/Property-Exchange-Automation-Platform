...
def _normalize_listing_times_value(value: str) -> str:
    ...
    if re.fullmatch(r"\d+", normalized):
        try:
            ...
            return normalized
        return _convert_listing_times_to_chinese(count) or normalized
    return normalized
        ...
        for key in keys:
            ...
            if value:
                if not os.path.isabs(value):
                    # Resolve relative paths against the ppe_config directory
                    config_dir = os.path.join(
                        os.path.dirname(os.path.abspath(__file__)),
                        os.pardir, os.pardir, "ppe_config",
                    )
                    candidate = os.path.normpath(os.path.join(config_dir, value))
                    if os.path.exists(candidate):
                        return candidate
                    # Also try resolving relative to ppe_config parent (for ../ppe_config/... paths)
                    config_parent = os.path.join(
                        os.path.dirname(os.path.abspath(__file__)),
                        os.pardir, os.pardir,
                    )
                    candidate2 = os.path.normpath(os.path.join(config_parent, value))
...
class R006DeriveListingTimesRule(_NoopRule):
    ...
    def apply(self, record: CanonicalRecord, context: RuleContext) -> RuleResult:  # noqa: ARG002
        ...
        patches: List[Patch] = []
        findings: List[Finding] = []
        normalized_current = _normalize_listing_times_value(current_value)
        ...
        elif normalized_current == derived:
            if current_value != normalized_current:
                patches.append(
                    Patch(
                        field=listing_field,
                        old_value=current_value,
                        ...
                        action="normalize",
                        reason="normalize_listing_times_format",
                    )
                )
        else:
            findings.append(
                Finding(
                    ...
                    message=f"listing_times conflict current={normalized_current or current_value} derived={derived}",
    ...
    def apply(self, record: CanonicalRecord, context: RuleContext) -> RuleResult:  # noqa: ARG002
        ...
        if project_code and listing_value:
            ...
            if suffix_match and int(suffix_match.group(1)) == 0:
                ...
                if derived_listing and _normalize_listing_times_value(listing_value) != derived_listing:
                    findings.append(
                        Finding(
                            rule_id=self.rule_id(),
                            severity="warn",
