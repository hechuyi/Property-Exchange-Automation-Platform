...
from peap_core.runtime import resolve_runtime_data_root as _resolve_runtime_data_root
from peap_core.source_catalog import canonical_source_code, list_source_descriptors
...
def _normalize_config_source_alias(value: object) -> str:
    text = str(value or "").strip()
    return str(canonical_source_code(text, allow_substring=False) or text).strip()


def _normalize_config_business_alias(value: object) -> str:
    text = str(value or "").strip()
    if text.lower() == "all":
        return "all"
    try:
        return get_business_descriptor(text).business_id
    except KeyError:
        return text


def _allowed_downloader_default_exchanges() -> set[str]:
    return {descriptor.source_id for descriptor in list_source_descriptors()} | {"all"}


def _allowed_downloader_default_business_ids() -> set[str]:
    return {descriptor.business_id for descriptor in list_business_descriptors()} | {"all"}
...
def _normalize_exchange_list(values: Iterable[str]) -> list[str]:
    normalized: list[str] = []
    seen: set[str] = set()
    for value in values:
        item = _normalize_config_source_alias(value)
...
def _normalize_exchange_name_mapping(values: Dict[str, str]) -> Dict[str, str]:
    normalized: Dict[str, str] = {}
    for key, value in values.items():
        normalized[_normalize_config_source_alias(key)] = value
    return normalized
...
class Config:
    ...
    def __init__(
        ...
        *,
        ...
        self.PROJECT_ROOT = _normalize_path(project_root or os.path.dirname(__file__))
    ...
    def _build_settings(
        ...
        *,
        ...
        data_root = _resolve_runtime_data_root(
            _require_string(paths, "data_root"),
            project_root=self.PROJECT_ROOT,
        )
        ...
        streaming_db_path = _resolve_path(
            _require_optional_string(paths, "streaming_db_path") or "streaming_ingest.sqlite3",
            base_dir=data_root,
        )
        ...
        output_files = {
            key: os.path.join(output_excel_dir, file_name)
            for key, file_name in output_file_names.items()
        }
        deal_files = {
            key: os.path.join(output_excel_dir, file_name)
            for key, file_name in deal_file_names.items()
        }
        ...
        supported_exchanges = _normalize_exchange_list(_require_string_list(
        ...
        exchange_names = _normalize_exchange_name_mapping(_require_string_mapping(
        ...
        business_id_key = "business_id" if "business_id" in downloader_defaults_raw else "project_type"
        downloader_defaults = {
            "exchange": _normalize_config_source_alias(_require_string(downloader_defaults_raw, "exchange")),
            "business_id": _normalize_config_business_alias(
                _require_string(downloader_defaults_raw, business_id_key)
            ),
        ...
        allowed_default_exchanges = _allowed_downloader_default_exchanges()
        if downloader_defaults["exchange"] not in allowed_default_exchanges:
            raise ValueError(
                "downloader_defaults.exchange must be one of: "
                f"{_format_allowed_values(allowed_default_exchanges)}"
            )
        allowed_default_business_ids = _allowed_downloader_default_business_ids()
        if downloader_defaults["business_id"] not in allowed_default_business_ids:
            raise ValueError(
                "downloader_defaults.business_id must be one of: "
                f"{_format_allowed_values(allowed_default_business_ids)}"
            )
        ...
        downloader_task_ids = tuple(DEFAULT_DOWNLOAD_TASK_PAGE_SIZE)
        downloader_page_size_resolved = dict(downloader_page_size_raw)
        for task_id, page_size in DEFAULT_DOWNLOAD_TASK_PAGE_SIZE.items():
            downloader_page_size_resolved.setdefault(task_id, page_size)
        _require_keys(
            downloader_page_size_resolved,
